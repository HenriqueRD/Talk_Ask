...
export function AdminRoom() {
    ...
    async function answeredQuestion(questionId: string) {
        const roomRef = await database.ref(`rooms/${params.id}/questions/${questionId}`);

        if((await roomRef.get()).val().isAnswer === false) {
    ...
    async function highLightedQuestion(questionId: string) {
        const roomRef = await database.ref(`rooms/${params.id}/questions/${questionId}`);

        if((await roomRef.get()).val().isHighLighted === false) {
            await (roomRef).update({
                isHighLighted: true,
            });
                            ...
                            questionRoom.map(event => {
                                ...
                                >
                                    <div className="buttonAdmin">
                                        {!event.isAnswer && 
                                            (                                    
                                                <button onClick={() => highLightedQuestion(event.id)} title="Dar destaque na pergunta" className="isHigh">
                                        ...
                                        <button onClick={() => answeredQuestion(event.id)} title="Marcar pergunta como respondida" className="isAnswer">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
